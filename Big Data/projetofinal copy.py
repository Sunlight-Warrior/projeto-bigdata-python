...
df_roubos = pd.read_csv('Pasta1.csv', sep=';', decimal=',')
df_populacao = pd.read_csv('original_utf8.csv')
df_salarios = pd.read_csv('Média e Mediana.csv', sep=';', decimal=',')
df_faixa_etaria = pd.read_csv('População_Residente_por_Sexo_Faixa_Etária.csv', sep=';', decimal=',', encoding='latin-1')

# Lendo o arquivo de imóveis que você enviou
...
class GraphSlider:
    def __init__(self):
        ...
        self.total_slides = 6  # Atualizado para 6 slides
    ...
    # SLIDE 2 — HOMENS x MULHERES (CORES ORIGINAIS: AZUL E ROSA)
    def create_gender_chart(self, ax):
        ...
        # CORES ORIGINAIS: Azul para homens, Rosa para mulheres
        bars_h = ax.bar(
            [x - largura/2 for x in x_pos],
            df_filtrado['Homens/2022'],
            width=largura,
            label='Homens',
            color='#1f77b4',  # Azul original
            edgecolor='black',
            alpha=0.9
        )
    ...
    # SLIDE 3 — HOMENS E MULHERES 15-50 ANOS (GRÁFICO HORIZONTAL AGRUPADO)
    def create_gender_15_50_chart(self, ax):
        ...
        # Ordenar por total (do menor para o maior para barras horizontais)
        df_filtrado = df_filtrado.sort_values('Total_15_50', ascending=True)
        
        y_pos = range(len(df_filtrado))
        altura = 0.35

        # CORES AZUL E ROSA: Homens e Mulheres
        bars_h = ax.barh(
    ...
    def create_property_value_chart(self, ax):
        ...
        for bar in bars:
            v = bar.get_width()
            ax.text(v + (v * 0.01),  # Pequeno offset para não colar na barra
                   bar.get_y() + bar.get_height()/2,
                   f'R$ {v:,.0f}'.replace(',', '.'),
                   va='center',
                   fontsize=12,
        ...
        ax.set_xlabel('Valor Médio do Imóvel (R$)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Bairros', fontsize=12, fontweight='bold')
        ax.grid(axis='x', linestyle='--', alpha=0.3)
        
        # Ajustar limite do eixo x para dar espaço para os textos
